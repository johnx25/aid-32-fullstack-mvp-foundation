...
For Supabase, use a pooled URL for `DATABASE_URL` and a direct PostgreSQL URL for `DIRECT_URL`.

## Commands run and outcomes

1. `npm run prisma:generate` (with `DATABASE_URL` set) -> PASS
2. `npx prisma validate` (with `DATABASE_URL` set) -> PASS
3. `npm run lint` (with `DATABASE_URL` set) -> PASS
4. `npm run build` (with `DATABASE_URL` set) -> PASS
5. `npm run prisma:migrate:deploy` (with `DATABASE_URL` + `DIRECT_URL` set) -> PASS on a reachable PostgreSQL/Supabase target
...
For Supabase verification, provide reachable Supabase `DATABASE_URL` and `DIRECT_URL` secrets and rerun the same commands.

## Additional notes

- `prisma.config.ts` imports `dotenv/config`, so Prisma CLI commands resolve `.env` values in this repository layout.
- `.env` and `.env.example` use PostgreSQL sample URLs for both `DATABASE_URL` and `DIRECT_URL`.
