...
5. `npm run prisma:migrate:deploy` (with `DATABASE_URL` set) -> PASS on a reachable PostgreSQL/Supabase target
...
For Supabase verification, provide a reachable Supabase `DATABASE_URL` secret and rerun the same commands.

## Additional notes

- `prisma.config.ts` imports `dotenv/config`, so Prisma CLI commands resolve `.env` values in this repository layout.
- `.env` and `.env.example` use PostgreSQL sample URLs via `DATABASE_URL`.
